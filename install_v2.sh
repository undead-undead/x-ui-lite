...
if [[ $LANG == "zh" ]]; then
    ...
    msg[xray_exists]="${green}检测到 Xray-lite 已存在，跳过安装${plain}"
    msg[xray_installing]="${green}正在安装 Xray-lite...${plain}"
    msg[xray_fail]="${red}Xray-lite 下载失败，请检查网络！${plain}"
    ...
    msg[xray_exists]="${green}Xray-lite detected, skipping installation${plain}"
    msg[xray_installing]="${green}Installing Xray-lite...${plain}"
    msg[xray_fail]="${red}Xray-lite download failed, check network!${plain}"
...
RELEASE_URL="https://github.com/undead-undead/x-ui-lite-v2/releases/download/v2.3.1/x-ui-linux-${arch}.tar.gz"
...
install_xray() {
    ...
    local xray_lite_url="https://github.com/undead-undead/x-ui-lite-v2/releases/download/v2.2.0/${xray_lite_file}"
    ...
    local keygen_url="https://github.com/undead-undead/x-ui-lite-v2/releases/download/v2.2.0/${keygen_file}"
